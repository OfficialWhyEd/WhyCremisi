...
```
╔══════════════════════════════════════════════════════════════════╗
║                    ARCHITETTURA WHYCREMISI                       ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │                  LAYER UI (React/WebView)                 │   ║
║  │  Chat · BoxChat · BotFace · Telemetry · Transport Panel  │   ║
║  └─────────────────────┬────────────────────────────────────┘   ║
...
║  │ Ollama  │    │ Analyzer    │    │ PersonalityCore │         ║
║  │ Gemini  │    │ Compressor  │    │ Memory System   │         ║
║  │ OpenAI  │    │ EQ · Limiter│    │ AgentSoul       │         ║
║  └────┬────┘    └──────┬──────┘    └────────┬────────┘        ║
║       │                │                    │                   ║
║  ┌────▼────────────────▼────────────────────▼────────────┐    ║
║  │                  LAYER DAW (JUCE)                       │    ║
║  │         PluginProcessor · PluginEditor                  │    ║
║  │         ParameterMapper · PluginChain                   │    ║
║  └─────────────────────┬────────────────────────────────────┘  ║
║                        │ OSC / VST3 Automation                  ║
║  ┌─────────────────────▼────────────────────────────────────┐  ║
║  │                  DAW HOST + PLUGIN TERZI                  │  ║
║  │    Ableton · Logic · Reaper · FabFilter · iZotope · UAD  │  ║
║  └──────────────────────────────────────────────────────────┘  ║
╚══════════════════════════════════════════════════════════════════╝
```
...
Il bridge è il cuore del sistema. Traduce tra il mondo JavaScript/React e il mondo C++/JUCE/DAW.
...
```
  ...
  5. AI risponde: "Ho rilevato un accumulo a 200Hz che intorbidisce
     la kick drum. Suggerisco un taglio narrow di -3dB a 200Hz con
     Q=4. Vuoi che lo applichi?"
  
  6. OscBridge: streaming risposta → UI via ai.stream chunks
  
  7. UI: BoxChat tipo 'eq' appare automaticamente (rileva "200Hz")
     mostrando lo spettro con il problema evidenziato
  
  8. Utente: "sì applica"
  
  9. AiEngine: determina azione → OscBridge → ParameterMapper
     → VST3 automation su FabFilter Pro-Q3 (trackId: 1, band: 2,
        frequency: 200Hz, gain: -3dB, Q: 4.0)
  
  10. AgentSoul: registra in memoria: "applicato EQ 200Hz -3dB kick,
      accettato dall'utente — riconoscere pattern in sessioni future"
```
...
*→ Continua in: [Paper 03 — Controllo Plugin di Terze Parti](03-CONTROLLO-PLUGIN-TERZE-PARTI.md)*
